# =============================================================
# 03_depth_estimate.py — Depth map generation
# =============================================================
# PURPOSE:
#   Third step. Generates a grayscale depth map from the
#   background-removed image. The depth map encodes how far
...
#   python 03_depth_estimate.py --file photo_nobg.png
...
#
# PROFILES (edge masking behaviour — see md_helpers/03-depth-guide.md):
...
#
# See md_helpers/03-depth-guide.md for the reasoning behind each profile.
...
def list_nobg_images(run: str) -> list[Path]:
    ...
    Only files ending in _nobg.png are included — mask files (_mask.png) are excluded.

    Args:
        run: Run subfolder name, e.g. 'try_03'
    ...
    images = sorted(
        [
            p for p in nobg_dir.iterdir()
            if p.is_file() and p.name.endswith("_nobg.png")
        ],
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Step 03 — Generate depth map with Depth Anything V2 / MiDaS / ZoeDepth.",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "See md_helpers/03-depth-guide.md for details."
        ),
    )
    parser.add_argument(
        "--feather",
...
def main() -> None:
    ...
    print("=" * 60)
    print("K9 Crystal Pipeline  —  Step 03: Depth Estimation")
    print("=" * 60)
    ...
    # Input: latest non-empty bg_removed run (or explicit --from-run)
    ...
    print()
    print("Next step: python 04_mesh_generate.py")
    print("=" * 60)
