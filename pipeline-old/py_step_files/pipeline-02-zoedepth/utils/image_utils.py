...
def compute_target_size(orig_w: int, orig_h: int, target_long_edge: int) -> tuple[int, int]:
    ...
    Always rounds to the nearest pixel on the short side. The long side is set
    exactly to target_long_edge. This is the single source of truth for all
    resize operations in the pipeline — never compute target sizes inline.

    Args:
        orig_w:           Original image width in pixels
        orig_h:           Original image height in pixels
        target_long_edge: Desired pixel length of the longer dimension

    Returns:
        (new_w, new_h) tuple — both dimensions are positive integers
    """
    if orig_h >= orig_w:
...
def resize_image(image: Image.Image, new_w: int, new_h: int) -> Image.Image:
    ...
    Works for any PIL mode — RGB, RGBA, L, etc. Always use compute_target_size()
    to derive new_w/new_h so the aspect ratio is guaranteed to be correct.

    Args:
        image: PIL Image in any mode
        new_w: Target width in pixels
        new_h: Target height in pixels
...
def extract_alpha_mask(rgba_image: Image.Image) -> Image.Image:
    """
    Extract the alpha channel from an RGBA image and return it as a grayscale image.

    White (255) = subject kept, black (0) = background removed.
    Used to save a visual mask alongside the RGBA result for inspection.

    Args:
        rgba_image: PIL Image in RGBA mode

    Returns:
        PIL Image in mode 'L' (grayscale), same dimensions as input
    """
