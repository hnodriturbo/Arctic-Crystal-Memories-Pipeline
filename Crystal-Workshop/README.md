...
# ACM pipelines and operator website

The requested parent-folder name is `Main-Pipelines-Web`; renaming is pending a
Windows lock held by existing Blender MCP runtimes. Active pipeline environments
remain separate. The new **Cockpit Reconstruct** menu reconstructs exported
DXF/CAD points as a continuous GLB/STL relief; see
[its operator guide](pipeline-converter/docs/cockpit-reconstruct.md).

Supporting project documents and learning material now live in `docs/` and
`Learning/` here. Obsolete top-level pipeline implementations have been removed;
original input collections were retained in the ignored reference gallery.
