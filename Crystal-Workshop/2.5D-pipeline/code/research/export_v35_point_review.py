...
def main():
    ...
    if args.layer_spacing_mm is not None:
        ...
        point_positions, retained, lattice = quantize_point_lattice(point_positions, args.surface_spacing_mm*.001, args.layer_spacing_mm*.001)
        point_uv = point_uv[retained]
        sampling.update(lattice)
        sampling['layer_axis'] = 'source-camera Z (near/far)'
        sampling['max_z_quantization_mm'] = lattice['max_axis_quantization_mm'][2]
