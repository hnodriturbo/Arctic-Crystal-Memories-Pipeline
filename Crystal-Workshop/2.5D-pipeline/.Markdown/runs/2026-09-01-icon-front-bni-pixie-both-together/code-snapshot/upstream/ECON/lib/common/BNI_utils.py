...
def adaptive_depth_fillet(depth_map, mask, radius, gradient_quantile):
    """Blend only the strongest depth discontinuities into short smooth ramps."""

    if radius <= 0:
        return depth_map

    depth = cp.asnumpy(depth_map).astype(np.float32)
    valid = cp.asnumpy(mask).astype(bool) & np.isfinite(depth)
    gradient = np.zeros_like(depth, dtype=np.float32)

    horizontal_valid = valid[:, 1:] & valid[:, :-1]
    horizontal = np.abs(depth[:, 1:] - depth[:, :-1])
    ...
    vertical = np.abs(depth[1:, :] - depth[:-1, :])
    ...
    candidates = gradient[np.logical_and(valid, gradient > 0.0)]
    if candidates.size == 0:
        return depth_map

    threshold = np.percentile(candidates, gradient_quantile)
    edge = np.logical_and(valid, gradient >= threshold).astype(np.uint8)
    kernel_size = radius * 2 + 1
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
    band = cv2.dilate(edge, kernel).astype(np.float32)
    ...
    result = depth.copy()
    result[valid] = (
        depth[valid] * (1.0 - blend[valid]) + smoothed[valid] * blend[valid]
    )
...
def double_side_bilateral_normal_integration(
    ...
    remove_stretched=True,
    fillet_radius=0,
    fillet_gradient_quantile=98.5,
):
    ...
    for i in range(max_iter):
        ...
        if False:
            ...
            if remove_stretched:
                ...
                faces_front_ = faces_front
                faces_back_ = faces_back
    ...
    if fillet_radius > 0:
        depth_map_front_est = adaptive_depth_fillet(
            depth_map_front_est,
            normal_mask,
            fillet_radius,
            fillet_gradient_quantile,
        )
