...
class BNI:
    def __init__(self, dir_path, name, BNI_dict, cfg, device):
        ...
        self.remove_stretched = self.cfg.get('remove_stretched', True)
        self.fillet_radius = self.cfg.get('fillet_radius', 0)
        self.fillet_gradient_quantile = self.cfg.get('fillet_gradient_quantile', 98.5)
    ...
    def extract_surface(self, verbose=True):
        ...
        bni_result = double_side_bilateral_normal_integration(
            ...
            remove_stretched=self.remove_stretched,
            fillet_radius=self.fillet_radius,
            fillet_gradient_quantile=self.fillet_gradient_quantile,
        )
        ...
        # Keep the two source-aligned surfaces available independently. The
        # official full reconstruction still uses the combined front/back mesh.
