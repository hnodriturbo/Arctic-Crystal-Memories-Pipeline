...
try {
  ...
  const routeModule = new vm.SourceTextModule(await fs.readFile(new URL('../src/app/api/r2/browser/route.js',import.meta.url),'utf8'),{context});
  await routeModule.link(name=>{const values=dependencies[name];assert.ok(values,name);return new vm.SyntheticModule(Object.keys(values),function(){for(const [key,value]of Object.entries(values))this.setExport(key,value);},{context});});
  await routeModule.evaluate();
  const get = query=>routeModule.namespace.GET(new Request('https://workshop.example/api/r2/browser?'+query));
