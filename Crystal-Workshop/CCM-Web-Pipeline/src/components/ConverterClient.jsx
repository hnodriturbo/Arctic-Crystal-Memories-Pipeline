...
export default function ConverterClient({
  ...
  const [values, setValues] = useState(() => ({
    ...
    ...([".glb", ".gltf"].includes(handoff?.file?.extension) ? {input_unit:"m"} : {}),
  ...
  function chooseSourceFile(file) {
    setSourceFile(file);
    setValues(current => ({...current, input_unit: [".glb", ".gltf"].includes(file.extension) ? "m" : "mm"}));
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <header className="space-y-1">
        <h1 className="text-2xl font-semibold">{t("Crystal Workshop")}</h1>
        <p className="max-w-3xl text-sm text-muted">
          {t("Upload a model, choose what happens to it, download something the SSLE engraver reads.")}
        </p>
      </header>
      <R2FileBrowser onUse={(data) => { chooseSourceFile(data.file); }} />
