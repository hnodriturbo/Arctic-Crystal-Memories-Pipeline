...
export const authConfig = {
  ...
  callbacks: {
    // A login callback must never point back to the login page or another origin.
    redirect({ url, baseUrl }) {
      try {
        const target = new URL(url, baseUrl);
        if (target.origin !== new URL(baseUrl).origin || /^\/login\/?$/.test(target.pathname)) return new URL('/', baseUrl).href;
        return target.href;
      } catch { return new URL('/', baseUrl).href; }
    },
