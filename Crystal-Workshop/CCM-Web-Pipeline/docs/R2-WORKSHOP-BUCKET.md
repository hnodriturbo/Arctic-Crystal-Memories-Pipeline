...
Purpose: The current ccm-workshop R2 bucket and preserved migration history.
-->

# R2 bucket `ccm-workshop`

## CCM migration — 26-09-2026

The authorized migration is active on Workshop. Both legacy buckets were copied
into the private EU bucket `ccm-workshop`. The verified inventory contains 264 pipeline objects and
271 design objects, with no overlapping keys. Keep `Cockpit3D-Files/`,
`Cockpit3D-Scene-History/`, `jobs/` and `claude-design/` unchanged. Both existing
`R2_PIPELINE_*` and `R2_WORKSHOP_*` variable groups use the same new bucket
and dedicated writer. All 535 destination bodies were SHA-256 verified during
the final writer pause before activation. No source data was deleted. Main's
separate read-only token is active; its library and exact asset downloads passed,
and a write-permission probe was denied. The bridge never uses this writer.

The current policy includes `https://workshop.ccm.is` in addition to the two
historical origins shown below. All methods, headers and the 3600-second cache
period match both old buckets and have been copied to the new bucket. Public
access stays disabled. The earlier migration intentions below are historical;
the task verification report records actual activation status.

## Historical setup before the CCM migration

The remaining sections record the earlier setup and are superseded by the
verified migration above. They are retained for recovery context.
