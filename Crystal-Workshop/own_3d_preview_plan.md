...
**`CCM-Web-Pipeline/src/components/CrystalPreview.jsx`** — three.js viewer that
...
**`CCM-Web-Pipeline`** — new sidebar step 3 with two tabs: *Build a relief* and
...
├── meshy-pipeline/       workspace · full 3D via the Meshy API (Node client lives in CCM-Web-Pipeline)
...
└── CCM-Web-Pipeline/     Next.js 16 · drives all four
```
...
| Form fields + argv | `CCM-Web-Pipeline/src/lib/relief/catalog.js` — a new flag is added here and nowhere else |
| Job orchestration, R2, pruning | `CCM-Web-Pipeline/src/lib/relief/{chain,library,state}.js` |
| Glass rendering | `CCM-Web-Pipeline/src/components/CrystalPreview.jsx` |
| Pipeline roots and venvs | `CCM-Web-Pipeline/src/lib/paths.js` |
