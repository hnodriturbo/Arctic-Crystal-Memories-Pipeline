...
class ConvertModelIntegrationTest(unittest.TestCase):
    ...
    def test_textured_glb_uses_metres_and_black_regions_have_no_points(self):
        import numpy as np
        import trimesh
        from PIL import Image
        import ezdxf
        with tempfile.TemporaryDirectory(prefix='acm-glb-density-test-') as temporary:
            source = Path(temporary) / 'portrait.glb'
            pixels = np.zeros((16, 16, 3), dtype=np.uint8)
            pixels[:, 8:] = 255
            mesh = trimesh.Trimesh(vertices=[[-.02,-.02,0],[.02,-.02,0],[.02,.02,.004],[-.02,.02,.004]], faces=[[0,1,2],[0,2,3]], process=False)
            mesh.visual = trimesh.visual.TextureVisuals(uv=[[0,0],[1,0],[1,1],[0,1]], image=Image.fromarray(pixels))
            source.write_bytes(mesh.export(file_type='glb'))
            command = [sys.executable, str(CONVERTER), '--file', str(source), '--out', temporary, '--formats', 'dxf', '--input-unit', 'm', '--photo-density', '--spacing', '1', '--min-distance', '1', '--layer-spacing', '1', '--max-points', '5000', '--seed', '7']
            completed = subprocess.run(command, capture_output=True, text=True, timeout=180)
            self.assertEqual(completed.returncode, 0, completed.stdout + completed.stderr)
            line = next(line for line in completed.stdout.splitlines() if line.startswith('ACM_CONVERTER_JOB='))
            job = json.loads(line.split('=',1)[1])
            directory = Path(job['directory'])
            report = json.loads((directory / 'job.json').read_text())
            self.assertAlmostEqual(report['processed']['dimensions_mm']['width'], 40, places=3)
            dxf = next(directory / file['path'] for file in job['files'] if file['extension'] == '.dxf')
            entities = list(ezdxf.readfile(dxf).modelspace())
            self.assertGreater(len(entities), 20)
            self.assertTrue(all(entity.dxftype() == 'POINT' for entity in entities))
            # UV density rejects the black left half before spacing/layer placement.
            left = sum(entity.dxf.location.x < -3 for entity in entities)
            right = sum(entity.dxf.location.x > 3 for entity in entities)
            self.assertGreater(right, 10 * max(left, 1))
