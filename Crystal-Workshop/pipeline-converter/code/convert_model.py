...
def run_blender(source, output_stem, job_dir, manifest_path, stage_obj, args):
    ...
    if args.photo_density and 'dxf' in args.formats:
        command.extend(['--density-texture', str(job_dir / '.acm-density-texture.png')])
...
def run_printer_dxf(stage_obj, job_dir, source_stem, args):
    ...
    if args.photo_density:
        command.extend(['--texture', str(job_dir / '.acm-density-texture.png'), '--texture-mode', 'uv', '--density-floor', '0'])
...
def build_parser():
    ...
    parser.add_argument("--photo-density", action="store_true", help="Use the single UV photo texture for DXF point density; black areas produce no points.")
    ...
    parser.add_argument("--layer-spacing", type=float, default=0.09)
