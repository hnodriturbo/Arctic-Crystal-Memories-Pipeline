...
def parse_arguments():
    ...
    parser.add_argument("--density-texture")
...
def save_density_texture(model, target):
    """Require one unambiguous portrait texture and UV map for laser density."""
    images = {}
    ...
    if len(images) != 1 or not model.data.uv_layers:
        raise ValueError('Photo density requires exactly one image texture and a UV map. Bake multiple materials to one texture in Blender first.')
    image = next(iter(images.values()))
    if image.packed_file:
        Path(target).write_bytes(image.packed_file.data)
    else:
...
def main():
    ...
    if args.density_texture:
        save_density_texture(model, args.density_texture)
