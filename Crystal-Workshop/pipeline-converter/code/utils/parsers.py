...
def parse_dxf_points_fast(file_path, limit=None, scale=1.0, sample_rate=1):
        ...
        for raw_code in it:
            ...
            if code == "2" and value == "ENTITIES":
                ...
                if code == "0":
                    ...
                    point = (current_x, current_y, float(value) * scale)
                    candidate_count += 1
                    if candidate_count % sample_rate == 0 and all(math.isfinite(v) for v in point):
                        points.append(point)
                    current_x = None
                    current_y = None
                    if limit is not None and len(points) >= limit:
