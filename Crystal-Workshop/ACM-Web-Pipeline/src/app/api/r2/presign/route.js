...
const PREFIXES = { uploads: "uploads", archive: "archive", showroom: "Cockpit3D-Files" };
...
export async function POST(request) {
  ...
  if (prefix === 'showroom' && extension !== '.glb') return Response.json({ error: 'The showroom library accepts GLB only.' }, { status: 400 });
  ...
  const key = `${folder}/${prefix === 'showroom' ? sceneFolder : day}/${Date.now()}-${safe}`;
