...
import { allowedStorageKey } from "@/lib/storage/browser-scope";
import { stat, rm } from "node:fs/promises";
import path from "node:path";

import { safeFileName, INPUT_DIR } from "@/lib/paths";
...
export async function POST(request) {
  ...
  const temporary = await createTemporaryWorkspace(INPUT_DIR, { sourceKeys: [normalized] });
  const destination = path.join(temporary.directory, fileName);
  ...
  try {
    ...
    return Response.json({
      file: {
        path: `${temporary.relative}/${fileName}`,
        name: fileName,
        extension,
        bytes: info.size,
        r2Key: normalized,
