...
import { copyFile, mkdir, stat, realpath } from "node:fs/promises";
...
export async function POST(request) {
  ...
  if (from === 'converter-output') {
    if (to !== 'converter-input' || extension !== '.glb') return new Response(null, {status:400});
    try {
      const actualRoot = await realpath(sourceRoot);
      const actual = await realpath(source);
      if (!resolveInside(actualRoot, path.relative(actualRoot, actual))) return new Response(null, {status:403});
      const info = await stat(actual);
      if (!info.isFile()) return new Response(null, {status:400});
      const temporary = await createTemporaryWorkspace(INPUT_DIR, { handoffSource: relativePath });
      await copyFile(actual, path.join(temporary.directory, path.basename(actual)));
      return Response.json({file:{path:temporary.relative + '/' + path.basename(actual),name:path.basename(actual),bytes:info.size,extension}});
    } catch { return Response.json({error:'The temporary GLB is unavailable. Recreate it or load its saved R2 version.'},{status:404}); }
  }
