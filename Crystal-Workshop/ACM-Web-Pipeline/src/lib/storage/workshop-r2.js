 ...
 * Purpose: The acm-workshop bucket - durable storage for everything this
 *          workshop owns that is not a pipeline job.
 *
 * Separate from storage/r2.js on purpose. That module speaks to acm-pipeline-eu,
 * which holds Cockpit scenes, Meshy jobs and converter output; this one speaks
 * to acm-workshop, created 20-09-2026 to own workshop storage from here on.
 * Two buckets, two credential sets, two clients - so a mistake in one prefix
 * can never write into the other bucket.
 *
...
function getClient() {
  if (!workshopR2Configured()) {
    throw new Error("The acm-workshop bucket is not configured. Add the R2_WORKSHOP_* keys.");
  }
  client ||= new S3Client({
    region: "auto",
