...
export default function PipelineSidebar({ active, onSelect, open = false, onClose, collapsed = false, onCollapse, badges = {} }) {
  const { t } = useLanguage();
  return (
    <>
      {open ? (
      ...
      <aside
        className={`${collapsed ? 'lg:hidden' : ''} fixed inset-y-0 left-0 z-40 flex w-72 shrink-0 flex-col border-r border-surface-border bg-surface-sunken transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:w-64 lg:translate-x-0 ${
      ...
      >
        ...
        <nav aria-label={t("Pipeline navigation")} className="flex-1 space-y-6 overflow-y-auto px-3 py-5">
          <button type="button" onClick={() => onSelect('home')} aria-current={active === 'home' ? 'page' : undefined} className="w-full rounded-xl border border-accent/25 bg-accent-soft px-4 py-3 text-left text-sm font-semibold text-accent-soft-text">Crystal Workshop</button>
