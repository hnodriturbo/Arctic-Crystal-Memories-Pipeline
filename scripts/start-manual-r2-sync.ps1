...
try {
    ...
    if ($task.State -eq 'Running') {
        ...
        [Windows.Forms.MessageBox]::Show('Expenses, Cockpit3D-Files, Claude Design and Income sync started. New CCM invoices are saved to ccm-bookkeeping and downloaded to local Income. Bank PDFs in Income sales-day settlement folders are uploaded too. Cockpit scenes and the Claude Design tree use ccm-workshop, which serves the Workshop library and Animations page. Large files can take several minutes. Logs are in CCM-Web-Workshop/deployment/artifacts/r2-sync-logs.', 'CCM R2 backup') | Out-Null
    }
} catch {
