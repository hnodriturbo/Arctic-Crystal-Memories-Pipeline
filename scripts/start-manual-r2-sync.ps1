...
try {
    ...
    if ($task.State -eq 'Running') {
        ...
        [Windows.Forms.MessageBox]::Show('Expenses, Cockpit3D-Files, Claude Design and Income sync started. New ACM invoices are saved to bookkeeping R2 and downloaded to local Income. Bank PDFs in Income sales-day settlement folders are uploaded too. The Claude Design tree is mirrored to the acm-workshop bucket, which is what the Animations page on pipeline.acm.is plays from. Large files can take several minutes. Logs are in ACM-Web-Workshop/deployment/artifacts/r2-sync-logs.', 'ACM R2 backup') | Out-Null
    }
} catch {
